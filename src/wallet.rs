use pqcrypto_dilithium::dilithium2::{
    keypair, sign, verify_detached, PublicKey, SecretKey, DetachedSignature, SignedMessage,
};
use pqcrypto_traits::sign::{DetachedSignature as DetachedSigTrait, SignedMessage as SignedMsgTrait};
...
impl Wallet {
    ...
    pub fn verify_signature(&self, message: &[u8], signature: &[u8]) -> bool {
        ...
        match DetachedSignature::from_bytes(signature) {
            Ok(sig) => verify_detached(&sig, message, &pk).is_ok(),
            Err(_) => false,
        }
    }
